...
export async function runQuinnAgent(db: Database.Database): Promise<QuinnResult> {
  ...
  try {
    ...
    const prompt = [
      ...
      '3. CRITICAL: Search results are NOT evidence. They are summaries written by an extraction LLM that may have hallucinated. The title, summary, and contacts in a search result could be WRONG.',
      '4. For EVERY search result that looks relevant, call prime_retrieve with the source_ref to read the ACTUAL email. You have 200 turns — USE THEM. Retrieve 15-25 sources per cycle, not 3-5. Read the actual emails. That is your job.',
      '5. Follow threads. Check who is actually IN the email thread, not who the summary mentions.',
      '6. Think strategically. Connect dots across projects and people.',
      '7. Your daily email to Zach will ONLY contain verified information. If you don\'t retrieve it, it won\'t be in the email. Unverified items are invisible to Zach. So retrieve everything that matters.',
      '',
      'PHASE 2 — CHALLENGE AND CLOSE GAPS:',
      'Before producing any output, stop and interrogate every conclusion you are about to present.',
      'THE RULE: If a claim is important enough to become an ACTION (something you tell Zach to DO), then you MUST have retrieved and read the actual source email. Go back and call prime_retrieve right now. Do not present an action based on a search summary.',
      'If a claim stays in "What I Think But Haven\'t Verified" — that\'s fine, flag it. But actions require evidence.',
      '',
      '',
      ...
      '',
      '## What I Verified This Cycle (ONLY items where I called prime_retrieve and read the full source)',
      '[STRICT RULE: A claim goes here ONLY if you called prime_retrieve with the source_ref and read the returned content. Search results, summaries, and PM reports are NOT verified — they go in the next section. If you did not call prime_retrieve for it, it is NOT verified. Period.]',
      '',
      '## What I Saw In Search Results But Did Not Verify',
      '[Search results, summaries, alerts, PM reports. These may be wrong — the extraction that produced them may have hallucinated. Say exactly what you saw and note that you did NOT read the original source. Example: "Search shows an item titled X from Apr 16 — I did not retrieve and read the actual email."]',
      '',
      '## Active Concerns',
      ...
      '- QUESTION AMBIGUOUS ANSWERS: If the knowledge base shows a "CEO Answer" or "Decision" entry where Zach said something casually (e.g., "Forrest can handle it"), do NOT treat that as a permanent ownership change. Cross-check against the actual email threads to see who REALLY owns the relationship. Decision log entries are Zach\'s intent filtered through an AI that may have misunderstood the context.',
      '- DECISION LOGS ARE NOT PRIMARY SOURCES: Corrections, decision entries, and user-feedback items were generated by the system\'s interpretation of what Zach said. The actual email threads are the ground truth. When a decision entry contradicts what the email threads show, trust the email threads.',
    ].filter(Boolean).join('\n');
