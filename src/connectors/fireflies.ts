...
export async function scanFireflies(
  ...
  // Fetch meetings list — filtered server-side, metadata only.
  // Full sentences are fetched per-meeting below, and only for NEW meetings.
  // (The old unbounded query pulled every transcript ever recorded with full
  // sentences; the response grew with the archive until Fireflies intermittently
  // errored with "Something unexpected happened".)
  console.log(`  Fetching meetings from ${fromDate} to ${toDate}...`);
  const data = await firefliesQuery(apiKey, `
    query($fromDate: DateTime, $toDate: DateTime, $limit: Int) {
  ...
  `, {
    ...
    limit: Math.min(maxMeetings, 50), // Fireflies API caps limit at 50
  });
  ...
  if (toProcess.length === 0) {
    ...
    db.prepare(
      `INSERT OR REPLACE INTO sync_state (source, last_sync_at, items_synced, status, updated_at)
       VALUES ('fireflies', datetime('now'), 0, 'idle', datetime('now'))`
    ).run();
    return stats;
  }
  ...
  for (let i = 0; i < toProcess.length; i += CONCURRENCY) {
    ...
    await Promise.all(batch.map(async (meeting: any) => {
      try {
        ...
        // Fetch the full transcript for THIS meeting only (list is metadata-only)
        let sentences: { text: string; speaker_name: string; start_time: number }[] = [];
        try {
          const tData = await firefliesQuery(apiKey, `
            query($id: String!) {
              transcript(id: $id) { sentences { text speaker_name start_time } }
            }
          `, { id: meeting.id });
          sentences = tData.transcript?.sentences || [];
        } catch (err: any) {
          console.error(`    transcript fetch failed for "${meeting.title}": ${err.message?.slice(0, 80)} — indexing from summary only`);
        }
        let transcriptText = '';
        let fullTranscript = '';
